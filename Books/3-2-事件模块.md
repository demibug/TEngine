...
TEngine 事件系统由四个核心组件构成：

| 组件 | 类型 | 职责 |
|------|------|------|
| **GameEvent** | 全局静态门面 | 持有 `static readonly EventMgr _eventMgr`（另有公开属性 `GameEvent.EventMgr`），所有方法委托给内部 EventMgr 实例 |
| **GameEventMgr** | 局部作用域管理器 | 实现 `IMemory`，用于非 UI 类需要批量管理事件的场景，仅有 `AddEvent` + `Clear()` |
| **GameEventHelper** | Source Generator 生成类 | 源码中无 .cs 文件，由 `[EventInterface]` 特性在编译时自动生成（仅当工程存在事件接口时），提供 `Init()` 注册所有接口事件 |
| **EventCenter** | Source Generator 生成类 | 源码中无 .cs 文件，按事件组封装的静态监听入口（`EventCenter.AddEvent.LoginUI.ShowLoginUI(...)` 等），组名为接口名去掉 `I` 前缀 |

### 两种事件模式对比

| 特性 | int/string 事件 | 接口事件 |
|------|----------------|---------|
| 定义方式 | `const int` 或 string（int 事件 ID 推荐来自生成类 `{接口名}_Event` 的字段） | 带 `[EventInterface]` 的接口 |
| 发送 | `GameEvent.Send(int/string, ...)` | `GameEvent.Get<IXxx>().OnXxx(...)` |
| 监听 | `GameEvent.AddEventListener` / `AddUIEvent` | `AddUIEvent({接口名}_Event.{方法名}, ...)` 或 `EventCenter.AddEvent...`（接口实现类由生成器自动注册） |
| 类型安全 | 无编译检查 | 编译期检查 |
| 事件 ID 来源 | 生成类字段或 `RuntimeId.ToRuntimeId` | Source Generator 自动生成 `IXxx_Event.OnXxx` |
| 适用场景 | 简单通知、UI 内部事件 | 模块间通信、多参数复杂事件 |

> **示意说明**：下文示例中的 `IBattleEvent` 为按生成约定书写的示意接口；当前工程真实存在的 `[EventInterface]` 接口仅有 `ILoginUI`（生成 `ILoginUI_Event`，事件 ID `ShowLoginUI`/`CloseLoginUI`）。
...
```csharp
// 返回接口实例，内部调用 _eventMgr.GetInterface<T>()；接口未注册时返回 null（不抛异常）
T GameEvent.Get<T>();
```
...
在 `UIBase` 子类（UIWindow/UIWidget）中使用，注册的事件**随窗口销毁自动清理**，无需手动 `RemoveEventListener`。仅有 `int` eventType 重载（无 string 版本）：

```csharp
// 在 RegisterEvent() 中使用，支持 0~4 个泛型参数
AddUIEvent(int eventType, Action handler);          // public
AddUIEvent<T>(int eventType, Action<T> handler);    // 以下为 protected
...
> **注意**：尽量使用接口事件，事件 ID 由 Source Generator 自动生成（见接口事件章节），无需手动定义。
...
{
    if (!ModuleSystem.IsRunning)                   // 模块系统关闭中则忽略
    {
        return;
    }
    GameEventHelper.Init();                        // ⚠️ 必须第一行！缺少此调用将导致所有接口事件无响应（无报错，极难排查）
    _hotfixAssembly = (List<Assembly>)objects[0];  // objects[0] 为 List<Assembly>
    RootModule.BeforeShutdown += Release;          // 注册关停回调
    Utility.Unity.AddDestroyListener(Release);     // 注册销毁回调
    StartGameLogic();
}
```
...
// 非 UI 类（GameEvent 有 string 重载，无需手动转换）
...
| `AddUIEvent`（UIBase） | 0~4 个泛型参数 | 不支持（仅 int 重载） |
