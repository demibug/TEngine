...
3. **🛡️ 内存泄漏检测**
   - 通过 `MemoryPool.EnableStrictCheck` 静态开关开启严格检查
   - 防止回收问题与内存泄漏问题
...
// ❌ 错误：Release 后继续访问对象
MemoryPool.Release(assetGroup);
assetGroup.Assets.Add(handle); // 危险！对象已被回收，内部状态已被 Clear()

// ❌ 错误：重复 Release 同一对象
MemoryPool.Release(assetGroup);
MemoryPool.Release(assetGroup); // 危险！会导致双重释放，破坏内存池状态

// ✅ 推荐：Release 后立即置空引用
MemoryPool.Release(assetGroup);
assetGroup = null; // 置空，防止误用
```

> **重要**：`Release` 后对象会被放回内存池并执行 `Clear()` 清理，继续访问或二次 `Release` 均会导致不可预期的 Bug，开启内存池检查功能可以在开发阶段检测此类问题。

---

## ⚙️ 配置说明

内存池严格检查通过静态属性 `MemoryPool.EnableStrictCheck` 控制，推荐使用 `MemoryPoolSetting` 组件配置：

将 `MemoryPoolSetting` 组件挂载到场景中的任意 GameObject（`Start` 时应用设置后自动销毁），可选检查模式：

- **AlwaysEnable** - 始终启用严格检查
- **OnlyEnableWhenDevelopment** - 仅开发构建启用（默认）
- **OnlyEnableInEditor** - 仅编辑器下启用
- **AlwaysDisable** - 始终禁用
