...
**默认命名规范**（完整前缀表，与 `ScriptGeneratorSetting.asset` 一致）：

| 前缀 | 生成类型 | 示例节点名 |
|------|---------|----------|
| `m_go` | `GameObject` | `m_goEffect` |
| `m_item` | `GameObject` 字段；作为根节点时生成 UIWidget 子类（其子节点不再递归） | `m_itemSlot` |
| `m_tf` | `Transform` | `m_tfContainer` |
| `m_rect` | `RectTransform` | `m_rectPanel` |
| `m_text` | `Text` | `m_textTitle` |
| `m_richText` | `RichTextItem`（该类型当前工程未实现，慎用） | `m_richTextDesc` |
| `m_btn` | `Button` | `m_btnStart` |
...
| `m_curve` | `AnimationCurve`（该类型当前工程未实现，慎用） | `m_curveAnim` |
...
> **注意**：
> - 前缀匹配为 `StartsWith` 首条命中（regex 不含尾部下划线），按 `ScriptGeneratorSetting.asset` 中的顺序进行；存在前缀包含关系时长前缀必须先声明（如 `m_scrollBar` 在 `m_scroll` 之前，`m_tmpInput`/`m_tmpDropdown` 在 `m_tmp` 之前）
> - 推荐统一连写风格（如 `m_goEffect`）；带下划线的 `m_go_Effect` 亦可命中 `m_go` 规则
> - **`m_canvas`/`m_dropdown` 未配置规则**（枚举中有 Canvas/Dropdown 成员但 setting 未添加），此类节点不会生成绑定，需先在 ScriptGeneratorSetting 中添加规则
...
代码生成有两种互斥模式，由 `ScriptGeneratorSetting` 的 `useBindComponent` 开关决定（当前工程默认启用 BindComponent 模式）：

**方式一：BindComponent 模式（当前工程默认，`useBindComponent = true`）**

在 UI Prefab 根节点挂载 `UIBindComponent` 组件，通过 Inspector 的「重新绑定组件」登记节点、「生成脚本」/「生成UniTask脚本」按钮生成 `Xxx_Gen.g.cs`（partial 字段声明 + 绑定代码）到 `UI/Gen` 目录，同时可选生成 partial 实现类。生成的绑定代码基于组件索引：

```csharp
// BindComponent 模式生成的绑定代码（按索引取组件）
...
**方式二：剪贴板模式（`useBindComponent = false`）**

在 UI 的根节点右键 `GameObject/ScriptGenerator` 菜单（如 `UIProperty`、`UIPropertyAndListener`，均可选 `- UniTask` 变体），生成 UI 绑定代码到剪贴板上，手动粘贴到自己的 UI 脚本中：

```csharp
// 剪贴板模式生成的绑定代码（按路径查找）
m_rectContainer = FindChildComponent<RectTransform>("m_rectContainer");
m_goTopInfo = FindChild("m_goTopInfo").gameObject;
m_btnStart = FindChildComponent<Button>("m_goTopInfo/m_btnStart");
...
namespace GameLogic
{
    ...
    class BattleMainUI : UIWindow
    {
        ...
        protected override void BindMemberProperty()
        {
            ...
            // 方式3：通过资源地址异步创建 Widget（在异步方法中 await 使用）
            // m_touchView = await CreateWidgetByPathAsync<TouchMove>(m_rectContainer, "TouchMove");
        }
...
namespace GameLogic
{
    ...
    class BagItemWidget : UIWidget
    {
        ...
        public void SetData(ItemData itemData)
        {
            ...
            // 设置物品图标（使用 SetSprite 自动管理资源；location 为纯文件名）
            m_imgIcon.SetSprite(itemData.IconName);
...
class TipWidget : UIWidget
{
    public static void ShowTip(string message, Vector2 position)
    {
        ...
        // 3 秒后自动销毁（GameModule.Timer 回调签名：void (object[] args)）
        GameModule.Timer.AddTimer(_ => tip.Destroy(), 3f);
    }
}
```
...
// 方式2：通过节点路径创建（在父 UI 下查找）
T CreateWidget<T>(string goPath, bool visible = true) where T : UIWidget, new();
T CreateWidget<T>(Transform parentTrans, string goPath, bool visible = true) where T : UIWidget, new();
...
UniTask<T> CreateWidgetByTypeAsync<T>(Transform parentTrans, bool visible = true) where T : UIWidget, new();

// 方式6：通过 Prefab 克隆创建
T CreateWidgetByPrefab<T>(GameObject goPrefab, Transform parentTrans = null, bool visible = true) where T : UIWidget, new();

// 列表数量调整（同步 / 异步分帧；异步版内部自动 Forget，返回 void）
void AdjustIconNum<T>(List<T> listIcon, int number, Transform parentTrans, GameObject prefab = null, string assetPath = "") where T : UIWidget, new();
void AsyncAdjustIconNum<T>(List<T> listIcon, int tarNum, Transform parentTrans, GameObject prefab = null, string assetPath = "", int maxNumPerFrame = 5, Action<T, int> updateAction = null) where T : UIWidget, new();
```

### 事件注册

```csharp
// 注册事件（自动绑定 UI 生命周期；首个重载为 public，其余为 protected）
