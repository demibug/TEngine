...
class WaveManager extends SingletonBase {
  ...
  configure({gameData,enemyManager,bossManager,eventBus,randomSource=Math.random,logger=console}={}){
    if(!gameData||!enemyManager||!bossManager||!eventBus)throw new TypeError('WaveManager requires gameData, enemyManager, bossManager and eventBus');
    Object.assign(this,{gameData,enemyManager,bossManager,eventBus,randomSource,logger});return this;
  }
  init(){}
  ...
  beginRound(round){const plan=this.planRound(round);if(plan.boss&&!plan.bossSpawned){this.bossManager.spawn(plan.bossKey,true);this.bossManager.spawn(plan.bossKey,false);plan.bossSpawned=true;const last=this.planHistory[this.planHistory.length-1];if(last&&last.round===round)last.bossSpawned=true;}return plan;}
