...
只生成 Configs/GameConfig/Datas/*.xlsx 数据表，并在 __tables__/__beans__ 追加注册行。
不修改 luban.conf / CustomTemplate / Defines / 导出脚本 / 生成代码。
重跑幂等：数据表覆盖重写，注册行按 full_name 去重追加。
"""
import json
import os
...
# 目标 Datas 目录
DATAS = r'E:\MyWork\MyTD\TEngine\Configs\GameConfig\Datas'
...
# 10. 武器名字拆字(多行)
add('weapon_text.xlsx', 'battle.TbWeaponText', 'WeaponText', '', '武器名字拆字',
...
def write_table(t):
    ...
    for row in t['rows']:
        ws.append(row)
    wb.save(os.path.join(DATAS, t['file']))
...
TBL_PATH = os.path.join(DATAS, '__tables__.xlsx')
...
BEAN_PATH = os.path.join(DATAS, '__beans__.xlsx')
