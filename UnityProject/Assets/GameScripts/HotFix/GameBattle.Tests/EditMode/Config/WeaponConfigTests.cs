...
namespace GameBattle.Tests.EditMode.Config
{
    ...
    //   1. 黄金目录（id 0～43）可加载：恰好四条 Basic +1 启用（0/10/20/31），
    //      其余 40 行 disabled 不产生运行时状态。
    //   2. 目录对外不可变：按 id 升序稳定排序、查询只读。
    //   3. 重复 id 在目录构造即被拒绝（Provider 转 WeaponIdDuplicate）。
    //   4. BasicWeaponResolver 显式映射 Knife→20、Bow→0、Spear→10、Cavalry→31，
    ...
    internal class WeaponConfigTests
    {
        ...
        [Test]
        [Description("黄金武器目录（id 0～43）可加载：恰好四条 Basic +1 启用（0/10/20/31）。")]
        public void GoldenCatalog_ExactlyFourBasicWeaponsEnabled()
        {
            ...
            Assert.IsTrue(catalog.ContainsId(0), "短弓 id=0 应存在");
            Assert.IsTrue(catalog.ContainsId(10), "短枪 id=10 应存在");
            Assert.IsTrue(catalog.ContainsId(20), "短刀 id=20 应存在");
            Assert.IsTrue(catalog.ContainsId(31), "短剑 id=31 应存在");

            Assert.IsTrue(catalog.TryGetById(0, out WeaponDefinitionSnapshot bow));
            Assert.AreEqual(WeaponType.Bow, bow.Type, "id=0 应为 Bow");
            Assert.IsTrue(catalog.TryGetById(31, out WeaponDefinitionSnapshot sword));
            Assert.AreEqual(WeaponType.Sword, sword.Type, "id=31 应为 Sword");
        }
        ...
        public void WeaponCatalogSnapshot_ImmutableAndSortedById()
        {
            ...
            {
                Def(31, WeaponType.Sword, 1, true, "Basic"),
                Def(0, WeaponType.Bow, 1, true, "Basic"),
                Def(10, WeaponType.Spear, 1, true, "Basic"),
                Def(20, WeaponType.Knife, 1, true, "Basic"),
            });

            Assert.AreEqual(4, catalog.Definitions.Count);
            Assert.AreEqual(0, catalog.Definitions[0].Id, "定义应按 id 升序");
            Assert.AreEqual(10, catalog.Definitions[1].Id);
            Assert.AreEqual(20, catalog.Definitions[2].Id);
            Assert.AreEqual(31, catalog.Definitions[3].Id);
            ...
            Assert.IsTrue(catalog.ContainsId(0));
        ...
        public void WeaponCatalogSnapshot_DuplicateId_ThrowsAtConstruction()
        {
            ...
            {
                Def(0, WeaponType.Bow, 1, true, "Basic"),
                Def(0, WeaponType.Bow, 2, false, null),
        ...
        {
            WeaponDefinitionSnapshot def = Def(20, WeaponType.Knife, 1, true, "Basic");

            Assert.AreEqual(20, def.Id);
        ...
        [Test]
        [Description("四类玩家兵种各解析到指定 ID 且 +1（Knife→20、Bow→0、Spear→10、Cavalry→31）。")]
        public void Resolver_ResolvesAllFourCategories()
        {
            BasicWeaponResolver resolver = new BasicWeaponResolver(GoldenCatalog());

            AssertResolves(resolver, SoldierType.Knife, 20, WeaponType.Knife);
            AssertResolves(resolver, SoldierType.Bow, 0, WeaponType.Bow);
            AssertResolves(resolver, SoldierType.Spear, 10, WeaponType.Spear);
            AssertResolves(resolver, SoldierType.Cavalry, 31, WeaponType.Sword);
        }

        [Test]
        [Description("显式映射不按枚举原始数值相等推断：Bow 解析到 id 0，Knife 解析到 id 20。")]
        public void Resolver_ExplicitMapping_NotByNumericEquality()
        {
            ...
            Assert.AreEqual(0, bow.Id, "Bow 必须解析到 id 0（短弓）");
        ...
        [Test]
        [Description("缺少任一启用行（如 id=0 未启用）→ 构造抛 InvalidOperationException。")]
        public void Resolver_MissingEnabledRow_Throws()
        {
            // id=0 改为 disabled。
            var catalog = new WeaponCatalogSnapshot(new[]
            {
                Def(0, WeaponType.Bow, 1, false, null),
                Def(10, WeaponType.Spear, 1, true, "Basic"),
                Def(20, WeaponType.Knife, 1, true, "Basic"),
                Def(31, WeaponType.Sword, 1, true, "Basic"),
            });
        ...
        public void Resolver_ExtraEnabledRow_Throws()
        {
            ...
            {
                Def(0, WeaponType.Bow, 1, true, "Basic"),
                Def(1, WeaponType.Bow, 2, true, null),
                Def(10, WeaponType.Spear, 1, true, "Basic"),
                Def(20, WeaponType.Knife, 1, true, "Basic"),
                Def(31, WeaponType.Sword, 1, true, "Basic"),
            });
        ...
        public void Resolver_WrongHandlerKey_Throws()
        {
            var catalog = new WeaponCatalogSnapshot(new[]
            {
                Def(0, WeaponType.Bow, 1, true, "Special"),
                Def(10, WeaponType.Spear, 1, true, "Basic"),
                Def(20, WeaponType.Knife, 1, true, "Basic"),
                Def(31, WeaponType.Sword, 1, true, "Basic"),
            });
        ...
        public void Resolver_WrongAttackPower_Throws()
        {
            var catalog = new WeaponCatalogSnapshot(new[]
            {
                Def(0, WeaponType.Bow, 2, true, "Basic"),
                Def(10, WeaponType.Spear, 1, true, "Basic"),
                Def(20, WeaponType.Knife, 1, true, "Basic"),
                Def(31, WeaponType.Sword, 1, true, "Basic"),
            });
        ...
        [Test]
        [Description("启用行类别与 id 期望不符（id=0 填 Knife）→ 构造抛 InvalidOperationException。")]
        public void Resolver_WrongWeaponTypeForId_Throws()
        {
            var catalog = new WeaponCatalogSnapshot(new[]
            {
                Def(0, WeaponType.Knife, 1, true, "Basic"),
                Def(10, WeaponType.Spear, 1, true, "Basic"),
                Def(20, WeaponType.Knife, 1, true, "Basic"),
                Def(31, WeaponType.Sword, 1, true, "Basic"),
            });
        ...
        /// <summary>获取黄金武器目录（id 0～43，与 weapon.xlsx 全量行等价）。</summary>
