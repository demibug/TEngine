...
namespace GameBattle
{
    ...
    internal sealed class BattleWorldHost
    {
        ...
        internal void ShowPlacementSlots(BattleMapState mapState, bool playerSide)
        {
            ...
            for (int x = 0; x < map.Width; x++)
            {
                for (int y = 0; y < map.Height; y++)
                {
                    ...
                    GridCell effectiveCell = mapState.GetEffectiveCell(position);
                    if (!effectiveCell.IsBuildable && !effectiveCell.IsCultivable)
                    {
                        continue;
                    }

                    GameObject terrain = new GameObject($"TerrainTile_{x}_{y}")
                    {
                        hideFlags = HideFlags.DontSave,
                    };
                    terrain.transform.SetParent(_bindings.UnitSlotRoot, false);
                    terrain.transform.position = _bindings.CellToWorld(x, y, 0.01f);
                    SpriteRenderer tileRenderer = terrain.AddComponent<SpriteRenderer>();
                    tileRenderer.sortingOrder = 7;
                    string terrainAddress = effectiveCell.IsCultivable
                        ? $"Sprites/Extracted/Map/grass_{map.MapIndex}_{ToLane(effectiveCell.Side)}"
                        : $"Sprites/Extracted/Map/space_{map.MapIndex}";
                    tileRenderer.SetSprite(
                        terrainAddress,
                        cancellationToken: terrain.GetCancellationTokenOnDestroy());
        ...
        /// <summary>原工程地图码 lane：0=玩家方，1=对手方。</summary>
        private static int ToLane(BuildableSide side)
        {
            return side == BuildableSide.Player ? 0 : 1;
        }
