...
{
    // ============================================================================
    // 任务 7.3：NullBattleAudioPort —— 纯逻辑 Null/Test 音频端口实现
    // ----------------------------------------------------------------------------
    // 职责（design.md 第 1 节目录表 / design.md:215）：
    //   IBattleAudioPort 的纯逻辑空实现，用于 EditMode 测试与无表现逻辑闭环。
    //   所有方法为空操作或立即返回，不执行任何 IO、不创建任何 AudioSource、
    //   不依赖 UnityEngine.Audio。使战斗模拟在无音频环境下可运行与测试。
    //
    // 来源证据：
    ...
    //
    // 使用方式：
    //   - EditMode 测试：BattleRuntimeFactory 注入 NullBattleAudioPort。
    ...
    //   5. 幂等：Clear / StopBgm 可重复调用。
    // ============================================================================

    /// <summary>
    /// <see cref="IBattleAudioPort"/> 的纯逻辑 Null/Test 实现，用于 EditMode 测试与无表现逻辑闭环。
    /// </summary>
    /// <remarks>
    /// <para><b>职责（design.md:215）：</b>所有方法为空操作或立即返回，不执行任何 IO。
    /// 使战斗模拟在无音频环境下可运行与测试。</para>
    ///
    /// <para><b>使用场景：</b></para>
    /// <list type="bullet">
    /// <item>EditMode 测试：注入本类型使逻辑闭环不依赖 Unity 音频。</item>
    ...
    /// </remarks>
    internal sealed class NullBattleAudioPort : IBattleAudioPort
    {
        // ====================================================================
        // 生命周期与预加载
        // ====================================================================
