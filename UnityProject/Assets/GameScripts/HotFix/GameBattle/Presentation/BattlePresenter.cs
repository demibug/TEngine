...
namespace GameBattle
{
    ...
    internal sealed class BattlePresenter : IDisposable
    {
        ...
        /// <summary>
        /// 按运行时 ID 解析活动战场单位的战场槽位标识（修复 P0 四向拖拽：战场源）。
        /// </summary>
        /// <param name="runtimeId">单位运行时 ID。</param>
        /// <param name="slotId">输出的战场槽位标识（-1 表示未命中）。</param>
        /// <returns>true=该单位当前有活动战场实例且在战场槽。</returns>
        internal bool TryResolveWorldUnitSlot(int runtimeId, out int slotId)
        {
            slotId = -1;
            if (IsDisposed || _unitRegistry == null)
            {
                return false;
            }

            SoldierBase soldier = _unitRegistry.GetUnit(runtimeId);
            if (soldier == null)
            {
                return false;
            }

            IReadOnlyList<UnitSlot> battleSlots = GetSlotSnapshot().GetSlots(
                soldier.Side, SlotZone.Battle);
            for (int i = 0; i < battleSlots.Count; i++)
            {
                BattleUnit? occupant = battleSlots[i].Occupant;
                if (occupant.HasValue)
                {
                    SoldierBase active = _unitRegistry.GetActiveByUnitId(occupant.Value.UnitId);
                    if (active != null && active.Id == runtimeId)
                    {
                        slotId = battleSlots[i].SlotId.Id;
                        return true;
                    }
                }
            }
        ...
        /// <summary>当前进行中的世界单位拖拽源槽位 ID；-1 表示未在拖拽。</summary>
        private int _worldDragSourceSlotId = -1;

        /// <summary>
        /// 开始拖拽一个世界单位（修复 P0 四向拖拽：Battle→Reserve / Battle→Battle）。
        /// </summary>
        /// <param name="runtimeId">单位运行时 ID。</param>
        internal void BeginWorldDrag(int runtimeId)
        {
            _worldDragSourceSlotId = -1;
            if (TryResolveWorldUnitSlot(runtimeId, out int slotId))
            {
                _worldDragSourceSlotId = slotId;
            }
        }

        /// <summary>
        /// 松手结束世界单位拖拽：把世界坐标转为战场格目标并提交 DropUnit（修复 P0）。
        /// </summary>
        /// <param name="worldPosition">Unity 世界坐标（鼠标抬起点）。</param>
        /// <returns>命令结果；未在拖拽或未命中目标返回 null。</returns>
        internal BattleInputResult? EndWorldDrag(UnityEngine.Vector3 worldPosition)
        {
            if (_worldDragSourceSlotId < 0)
            {
                return null;
            }

            int sourceSlotId = _worldDragSourceSlotId;
            _worldDragSourceSlotId = -1;
            ...
            UnityEngine.Vector3 screen = camera.WorldToScreenPoint(worldPosition);
            // FairyGUI Stage 左上角原点：Y 翻转。
            float stageX = screen.x;
            float stageY = UnityEngine.Screen.height - screen.y;

            if (!TryResolvePlayerBattleSlot(stageX, stageY, out int targetSlotId)
                || targetSlotId < 0
            ...
            return InputAdapter.HandleDropUnit(sourceSlotId, targetSlotId);
        }
