...
- **禁止使用复合 Bash 语句**：不要把多条命令用 `&&`、`;`、`|`、`||` 串联成一条。每条 Bash 调用只发**一条命令**，需要多步就分多次调用。
  - 原因：权限引擎对复合命令按子命令逐个匹配，只要有一个子命令未命中放行规则，整条就会弹窗。单条命令更容易命中规则或走内置只读放行，从而减少弹窗。
  - 反例：`cd X && echo y; tail f | iconv ... | tail`（一条复合，易弹窗）
  - 正例：分三次调用，分别执行 `cd`、`tail`、`iconv`。
- **只读命令优先单独使用**：`ls`/`cat`/`grep`/`git diff`/`git log`/`head`/`tail` 等只读命令单独成条时走内置放行，不依赖 allow 规则，不会弹窗。
- **需要管道时**：尽量拆成"一条产出文件、下一条读文件"两步，避免 `|` 管道。确需管道时接受可能弹窗一次，批准即可，不要因此重新堆砌具体命令到 allow。
- **不要把 `echo "exit=$?"` 之类追加到命令末尾**查退出码——分开调用或直接看命令返回值。
