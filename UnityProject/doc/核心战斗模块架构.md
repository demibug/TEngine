...
| 输入 | `BattleInputController`、`BattleInputCommand` | 输入命令进入单局运行时并由逻辑阶段处理；命令只剩 `Recruit` 与 `DropUnit` |
| 槽位/征兵/经济 | `UnitSlotBoard`、`RecruitManager`、`UnitLevelService`、`BattleEconomy` | 固定槽位（待上场/战场）、征兵批次、等级倍率、金币与单局经济规则（旧 `DeckManager` 牌组已移除，改由 `RecruitManager` 生成 1 级四兵批次） |
