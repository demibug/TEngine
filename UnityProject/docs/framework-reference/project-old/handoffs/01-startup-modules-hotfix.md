...
## Objective
以参考工程真实代码为依据，研究“启动、模块组织与热更边界”，沉淀可以独立复查的框架研究文档，为之后选择性复刻优秀设计提供依据。本轮只调查和写文档，不实现改造，不迁移任何具体游戏功能。

参考工程（只读）：`D:\Work\SAUnity\ProjectOld`
当前工程：`E:\MyWork\MyFramework\TEngine\UnityProject`
唯一允许写入的目录：`E:/MyWork/MyFramework/TEngine/UnityProject/docs/framework-reference/project-old/01-startup-modules-hotfix`
本提示词本身不需要修改。
...
### 本任务范围
负责运行时整体分层、程序集依赖、启动入口、模块注册与访问、初始化顺序、帧驱动、退出销毁，以及主包与热更代码的加载和桥接。追踪至少一条从实际入口到框架可用的调用链，并检查结束或退出路径。
重点回答：哪些层拥有模块；依赖如何表达；初始化失败如何处理；跨程序集类型怎样交互；热更接口受什么约束；旧代码或多套入口是否仍可达。
资源模块内部交给 02，UI 内部交给 03，事件/异步通用机制交给 04，构建与生成工具交给 06；这里只记录它们与启动/模块层的接口。

### 统一证据格式
- 所有文档使用中文。每项关键架构结论及候选建议引用证据编号 `S01-E001` 等。
...
- `D:\Work\SAUnity\ProjectOld\Assets\Scripts\Launcher`
- `D:\Work\SAUnity\ProjectOld\Assets\Scripts\HotUpdate`
- `D:\Work\SAUnity\ProjectOld\Assets\Scripts\framework`
- `D:\Work\SAUnity\ProjectOld\ILScriptProject\Main.cs`
...
当前工程的按需对照入口：
- `E:\MyWork\MyFramework\TEngine\UnityProject\Assets\GameScripts\GameEntry.cs`
- `E:\MyWork\MyFramework\TEngine\UnityProject\Assets\GameScripts\Procedure`
...
3. 先建立本领域机制清单，再沿代表性入口追踪实现、调用者和配置，覆盖上述重点问题。至少给出程序集依赖图、启动顺序图，以及模块创建/更新/销毁的所有权说明。
...
5. 在唯一允许目录内写入下列文件：
   ...
   - candidates.md：候选编号 S01-C001 等；记录设计、证据、当前等价能力、收益、代价、约束、跨领域依赖和建议优先级。不要为凑数量提出候选。
...
- Original objective：研究 启动、模块组织与热更边界 的实际框架机制，为后续选择性复刻提供依据，不迁移业务。
- Important implementation contract：OLD=D:\Work\SAUnity\ProjectOld；CURRENT=E:\MyWork\MyFramework\TEngine\UnityProject；仅写 E:/MyWork/MyFramework/TEngine/UnityProject/docs/framework-reference/project-old/01-startup-modules-hotfix；事实与推断分开，关键结论有证据，建议需对照当前框架。
