...
## Objective
以参考工程真实代码为依据，研究“资源与场景生命周期”，沉淀可以独立复查的框架研究文档，为之后选择性复刻优秀设计提供依据。本轮只调查和写文档，不实现改造，不迁移任何具体游戏功能。

参考工程（只读）：`D:\Work\SAUnity\ProjectOld`
当前工程：`E:\MyWork\MyFramework\TEngine\UnityProject`
唯一允许写入的目录：`E:/MyWork/MyFramework/TEngine/UnityProject/docs/framework-reference/project-old/02-resources-scenes`
本提示词本身不需要修改。
...
### 本任务范围
负责资源定位、加载接口、同步/异步路径、缓存、句柄/引用计数、实例化、释放、资源包边界及场景切换与资源生命周期的协作。
重点回答：资源与实例由谁持有和释放；共享引用如何处理；失败、取消、重复请求、卸载后回调如何处理；对象销毁是否必然释放资源。代码未实现的保障要写明，不能自行补全。
追踪至少一条加载到释放的完整链路及一条场景切换链路；没有对应机制时记录检索范围与证据缺口。
UI 只作为真实调用者抽样，不分析窗口架构；资源打包编辑器工具交给 06，热更代码加载入口交给 01。

### 统一证据格式
- 所有文档使用中文。每项关键架构结论及候选建议引用证据编号 `S02-E001` 等。
...
- `D:\Work\SAUnity\ProjectOld\Assets\SubSystem\ResLoadCtrl`
...
3. 先建立本领域机制清单，再沿代表性入口追踪实现、调用者和配置，覆盖上述重点问题。至少给出资源加载/释放时序、所有权表、场景切换涉及的清理边界。
...
5. 在唯一允许目录内写入下列文件：
   ...
   - candidates.md：候选编号 S02-C001 等；记录设计、证据、当前等价能力、收益、代价、约束、跨领域依赖和建议优先级。不要为凑数量提出候选。
...
- Original objective：研究 资源与场景生命周期 的实际框架机制，为后续选择性复刻提供依据，不迁移业务。
- Important implementation contract：OLD=D:\Work\SAUnity\ProjectOld；CURRENT=E:\MyWork\MyFramework\TEngine\UnityProject；仅写 E:/MyWork/MyFramework/TEngine/UnityProject/docs/framework-reference/project-old/02-resources-scenes；事实与推断分开，关键结论有证据，建议需对照当前框架。
