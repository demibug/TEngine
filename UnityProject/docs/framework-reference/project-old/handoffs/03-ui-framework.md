...
## Objective
以参考工程真实代码为依据，研究“UI 框架与表现层组织”，沉淀可以独立复查的框架研究文档，为之后选择性复刻优秀设计提供依据。本轮只调查和写文档，不实现改造，不迁移任何具体游戏功能。

参考工程（只读）：`D:\Work\SAUnity\ProjectOld`
当前工程：`E:\MyWork\MyFramework\TEngine\UnityProject`
唯一允许写入的目录：`E:/MyWork/MyFramework/TEngine/UnityProject/docs/framework-reference/project-old/03-ui-framework`
本提示词本身不需要修改。
...
### 本任务范围
负责 UI 基类、窗口/组件职责、管理器、层级与导航、创建/显示/隐藏/关闭/销毁生命周期、绑定机制、UI 内部事件清理，以及表现状态与业务状态的边界。
重点回答：窗口是否缓存或复用；关闭是否等于销毁；异步打开中关闭怎样处理；重复打开如何处理；事件、计时器、资源由谁清理；扩展新窗口需要哪些约定。
追踪至少一条实际窗口从打开到最终清理的调用链。业务窗口仅作为调用证据，不整理窗口功能、界面资源、业务数据或业务实现。
资源内部交给 02，通用事件实现交给 04，编辑器生成绑定流程交给 06；本任务保留运行时接口和约束。

### 统一证据格式
- 所有文档使用中文。每项关键架构结论及候选建议引用证据编号 `S03-E001` 等。
...
- `D:\Work\SAUnity\ProjectOld\Assets\Scripts\UIBase`
- `D:\Work\SAUnity\ProjectOld\Assets\Scripts\framework`
- `D:\Work\SAUnity\ProjectOld\Assets\Scripts\game`
...
- `E:\MyWork\MyFramework\TEngine\UnityProject\Assets\Launcher`
...
3. 先建立本领域机制清单，再沿代表性入口追踪实现、调用者和配置，覆盖上述重点问题。至少给出窗口生命周期图、基类/组件边界和一条可追溯的清理链。UIProject 仅检查必要入口，不全量扫描素材。
...
5. 在唯一允许目录内写入下列文件：
   ...
   - candidates.md：候选编号 S03-C001 等；记录设计、证据、当前等价能力、收益、代价、约束、跨领域依赖和建议优先级。不要为凑数量提出候选。
...
- Original objective：研究 UI 框架与表现层组织 的实际框架机制，为后续选择性复刻提供依据，不迁移业务。
- Important implementation contract：OLD=D:\Work\SAUnity\ProjectOld；CURRENT=E:\MyWork\MyFramework\TEngine\UnityProject；仅写 E:/MyWork/MyFramework/TEngine/UnityProject/docs/framework-reference/project-old/03-ui-framework；事实与推断分开，关键结论有证据，建议需对照当前框架。
