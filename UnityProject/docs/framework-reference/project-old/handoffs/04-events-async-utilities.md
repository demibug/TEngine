...
## Objective
以参考工程真实代码为依据，研究“事件、异步与通用运行时机制”，沉淀可以独立复查的框架研究文档，为之后选择性复刻优秀设计提供依据。本轮只调查和写文档，不实现改造，不迁移任何具体游戏功能。

参考工程（只读）：`D:\Work\SAUnity\ProjectOld`
当前工程：`E:\MyWork\MyFramework\TEngine\UnityProject`
唯一允许写入的目录：`E:/MyWork/MyFramework/TEngine/UnityProject/docs/framework-reference/project-old/04-events-async-utilities`
本提示词本身不需要修改。
...
### 本任务范围
负责跨模块通信、事件分发/订阅、异步任务与取消、计时器、通用状态机、对象池及相关生命周期机制。
重点回答：谁拥有订阅与任务；回调异常如何传播；重入或分发期间取消订阅如何处理；计时器与任务怎样退出；对象池如何重置与防止错误复用；线程或主线程约束是否实际保证。
至少完整追踪事件订阅到解绑，以及一个已存在的任务/计时器/池对象从创建到清理的真实调用链。若某机制未找到，如实报告。
启动模块的整体组织交给 01；UI、资源、网络只作为调用方样例，不展开内部实现。避免列出全部工具类；仅研究有实际使用证据的机制。

### 统一证据格式
- 所有文档使用中文。每项关键架构结论及候选建议引用证据编号 `S04-E001` 等。
...
3. 先建立本领域机制清单，再沿代表性入口追踪实现、调用者和配置，覆盖上述重点问题。至少提供事件生命周期与异常/重入行为说明，以及通用机制的所有权与清理表。
...
5. 在唯一允许目录内写入下列文件：
   ...
   - candidates.md：候选编号 S04-C001 等；记录设计、证据、当前等价能力、收益、代价、约束、跨领域依赖和建议优先级。不要为凑数量提出候选。
...
- Original objective：研究 事件、异步与通用运行时机制 的实际框架机制，为后续选择性复刻提供依据，不迁移业务。
- Important implementation contract：OLD=D:\Work\SAUnity\ProjectOld；CURRENT=E:\MyWork\MyFramework\TEngine\UnityProject；仅写 E:/MyWork/MyFramework/TEngine/UnityProject/docs/framework-reference/project-old/04-events-async-utilities；事实与推断分开，关键结论有证据，建议需对照当前框架。
