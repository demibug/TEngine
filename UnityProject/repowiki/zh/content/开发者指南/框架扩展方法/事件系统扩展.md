...
> ⚠️ 本文为历史分析文档：事件系统已升级为「跨程序集 Registrar 模式」（`GameEventHelper` 为手写类、`EventCenter` 废弃并有 `[Obsolete]` 兼容壳、事件接口支持契约见 `tengine-dev/references/event-system.md` 的「事件接口支持契约」章节）。文中「生成 GameEventHelper/EventCenter」的描述已过时，请以实际代码与参考文档为准。
